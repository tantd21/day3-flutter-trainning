...
float calculateTotalForInvoice(const std::vector<BangHoaDon>& hoadons, const std::string& soHD) {
    float total = 0.0f; // Khởi tạo tổng tiền bằng 0

    for (const auto& hoadon : hoadons) {
        if (hoadon.getsoHD() == soHD) {
            // Tính tổng tiền của hóa đơn và cộng vào biến total
            // Giả sử có một phương thức getTongTien() để lấy tổng tiền của hóa đơn
            total += hoadon.getTongTien();
        }
    }
    return total; // Trả về tổng tiền của hóa đơn
}